...
# Database connection
q = x.connect(host='localhost', user='root', passwd='dps', database='sm12c.db') 
cur = q.cursor() 

# Initial global data structures
d = {
    's': [1, 2, 3, 4, 5], 
    'n': ['Andrea Bell', 'Naomi Ward', 'Salil Singh', 'Nikitha Green', 'Percy Vere'], 
    'ar': ['Bristol', 'Warton', 'Cardigan', 'Skelton', 'London'], 
    'p': ['7450098033', '7036210126', '7399157138', '7911250314', '7508017277'], 
    'a': ['Backwell', 'Carnforth', 'Ambleston', 'Haxby', 'Wembley']
}

d3 = {
    's': 'sno', 'n': 'name', 'ar': 'area', 'p': 'phone_no', 
    'a': 'address', 'sm': 'manager_sno', 'sl': 'sal'
}

l7 = [
    [1, 'Andrea Bell', '2', '25000'],
    [2, 'Naomi Ward', 'NULL', '35000'],
    [3, 'Salil Singh', '2', '30000'],
    [4, 'Nikitha Green', '1', '20000'],
    [5, 'Percy Vere', '3', '15000']
] 
...
# Attempt to initialize data from the active database
try:
    t, t2 = sh(), sh('t111') 
    if len(t) > 0: 
        d1, l7 = {}, [] 
...
# Main CLI Loop
while True: 
    print('''\n0.Revamp database 1.add 2.show 3.update 4.delete 5.search 6.filter 7.sort 8.exit\n''') 
    x_opt = int(input('enter option\n')) 
    d = di('s', None) 
    ...
    elif x_opt == 2:  # show records--[format] 
        print('''\t1.employee personal data \t2.manager and salaries \t3.both''') 
    ...
    elif x_opt == 3:  # update records-(sql) 
        x1 = input('''\n\n\tenter what is to be updated: serial number s name n phone p area ar address a manager\'s serial number sm salary sl cancel c\n\t''') 
        if x1 in ('s', 'n', 'p', 'ar', 'a', 'sm', 'sl'): 
            if x1 != 'sm' and x1 != 'sl': 
                l_val, L = d.get(x1), d.get('s') 
                if x1 != 's': 
                    print('\t', d3[x1], d3['s'], sep='\t\t') 
                    print() 
                    for i in range(len(l_val)): 
                        if len(l_val[i]) > 8: 
                            print('\t\t', l_val[i], L[i], sep='\t') 
                        else:
                            print('\t', l_val[i], L[i], sep='\t\t') 
                else: 
                    print('\t\t', 'sno') 
                    ...
                    if x1 == 's': 
                        ...
                        for iu in l7: 
                            if iu[0] == k: 
                                iu[1] = l_val[r] 
                    d[x1] = l_val 
                    cur.execute("update t11 set '%s'='%s' where sno='%d';" % (d3[x1], tr, k)) 
            else: 
                if x1 == 'sm': 
                    ...
                    for ir in l7: 
                        if ir[0] == k: 
                            ir[3] = tr 
                if x1 in ('s', 'n', 'sm', 'sl') and k in L:
                    cur.execute("update t111 set '%s'='%s' where sno='%d';" % (d3[x1], tr, k)) 
                else: 
                    pass 
                    
    elif x_opt == 4:  # delete records-(sql) 
        print(d['s'], d['n']) 
    ...
    elif x_opt == 5:  # search records--[format] 
        x1 = int(input('\tenter S.No.\n\t')) 
        sear(x1) 
        if x1 in d['s']: 
            for ii in l7: 
    ...
    elif x_opt == 6:  # filter through records--[format] 
        while True: 
            print('''\t1.by character in name \t2.by ph number \t3.by area \t4.Exit Filter''') 
    ...
    elif x_opt == 7:  # sort the records--[format] 
        while True: 
            print('''\t1.by length of name \t2.by alphabetical order of name \t3.by phone number \t4.Exit Sort''') 
            a = int(input('\tenter option\n\t')) 
            if a in (1, 2, 3): 
                bo = int(input('''\t\t1.acsending \t\t2.descending\n\t\t''')) 
                if bo == 1:
                    b = False 
                elif bo == 2:
                    b = True 
